...
def create_a_table(date_table=''): #создает таблицу
...
def writing_to_the_table(namee): #запись в таблицу
    ...
    for i in balance_buratino.keys():
        if balance_buratino[i] != 0:
            ...
            elif i == 'Твердый переплет Переплет':
...
def pars_buratino(name, date): #основной цикл
